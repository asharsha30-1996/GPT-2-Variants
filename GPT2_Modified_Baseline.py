...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.num_groups = config.n_head // 2
        self.c_attn = nn.Linear(config.n_embd, 2 * config.n_embd + (config.n_embd // config.n_head) * self.num_groups)
        # output projection
        ...
        self.group_head_dim = self.head_dim
    ...
    def forward(self, x):
        ...
        qkv = self.c_attn(x)
        q, k, v = torch.split(qkv, [(self.n_embd//self.n_head * self.num_groups), self.n_embd, self.n_embd], dim=2)
        k = k.view(B, T, self.n_head, self.head_dim).transpose(1, 2)  # (B, nh, T, hs)
        v = v.view(B, T, self.n_head, self.head_dim).transpose(1, 2)  # (B, nh, T, hs)
        q = q.view(B, T, self.num_groups, self.group_head_dim).transpose(1, 2)  # (B, num_groups, T, group_head_dim)
        q = q.repeat(1, 2, 1, 1) 
        if FLASH:
            # flashattention for speed optimization
            ...
            y = att @ v  # (B, num_groups, T, group_head_dim)

        # Re-assemble all group outputs side by side
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_logits=True):
        ...
        #x = tok_emb + pos_emb
        inv_freq = 1.0 / (10000 ** (torch.arange(0, self.config.n_embd, 2, device=device).float() / self.config.n_embd))
        sin = torch.sin(pos * inv_freq).unsqueeze(0).expand(b, -1, -1)  # (b, t, n_embd//2)
        cos = torch.cos(pos * inv_freq).unsqueeze(0).expand(b, -1, -1)
        x = self.apply_rope(tok_emb,sin,cos)
...
if __name__ == "__main__":
    ...
    if args.model[0] == "d":
        ...
        model_config = {
            "d12": GPTConfig(block_size=2048, vocab_size=50257, n_layer=16, n_head=16, n_embd=1024),
    ...
    for step in range(args.num_iterations + 1):
        ...
        if step in [1000,5000,8000,9000,9500, 10000,12000,15000]:
            save_path = f"{args.output_dir}/model_checkpoint_{step}.pt"
    ...
    if master_process and logfile is not None:
        with open(logfile, "a") as f:
            f.write(f"final {len(timings)} iters avg: {np.mean(timings)*1000:.3f}ms\n")
