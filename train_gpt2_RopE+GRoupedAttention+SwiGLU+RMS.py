...
class SwiGLU(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.proj1 = nn.Linear(config.n_embd, 4 * config.n_embd)
        self.proj2 = nn.Linear(config.n_embd, 4 * config.n_embd)
        self.proj_out = nn.Linear(4 * config.n_embd, config.n_embd)

    def forward(self, x):
        return self.proj_out(F.silu(self.proj1(x)) * self.proj2(x))
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        rms = x.norm(2, dim=-1, keepdim=True).clamp(min=self.eps).mean()
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.rotary_embedding = RotaryEmbedding(self.n_embd // self.n_head)
...
class MLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.swi_glu = SwiGLU(config)

    def forward(self, x):
        return self.swi_glu(x)

# Block with RMSNorm and SwiGLU
class Block(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.rms_1 = RMSNorm(config.n_embd)  # RMSNorm
        self.attn = CausalSelfAttention(config)
        self.rms_2 = RMSNorm(config.n_embd)  # RMSNorm
        self.mlp = MLP(config)
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            ln_f = RMSNorm(config.n_embd),  # Use RMSNorm here too
        ))
